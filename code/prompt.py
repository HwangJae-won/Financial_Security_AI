    ...
    if is_mc:
        ...
        if contexts:
            return (
                role
                + "### 지시\n"
                "- [참고자료]에 근거하여 [질문]에 대한 최종 답변만 작성하세요.\n"
                "- 출력은 반드시 **두 줄**입니다.\n"
                "  - `답변: <한두 문장, 200자 이내>`\n"
                "  - `근거: <[참고자료]의 원문 한 문장>`\n"
                "- 법령·조문 번호가 보이면 명시하고, [참고자료]에 없는 내용은 추정하지 마세요.\n"
                "- 불필요한 문장, 불릿, 사고과정(CoT) 출력 금지.\n\n"
            ...
            return (
                role
                + "### 지시\n"
                "- [질문]에 대한 최종 답변만 간결히 작성하세요(한두 문장, 200자 이내).\n"
                "- [참고자료]가 없으므로 법령 명칭/조문/수치 등은 **추정하지 말고**, 일반 원칙 수준에서만 답하세요.\n"
                "- 출력은 반드시 **한 줄**, 형식은 `답변: <내용>`만 허용됩니다.\n"
                "- 사고과정(CoT), 불릿, 추가 문장 출력 금지.\n\n"
                + (fewshot_block_gen if use_fewshot else "")
                + "### 질문\n"
                + f"{text}\n\n"
                + "답변:"
